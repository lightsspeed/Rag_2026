    ...
    async def retrieve(self, query: str, top_k: int = 5, filters: Optional[Dict] = None) -> List[Dict]:
        ...
        try:
            ...
            for doc in full_docs:
                # Try new content field first (safely check if column exists in DB)
                content = None
                if hasattr(doc, 'content'):
                    content = doc.content
                
                if not content:
                    # Fallback: Join all chunks in order
                    chunks = db.query(models.Chunk).filter(models.Chunk.document_id == doc.id).order_by(models.Chunk.id).all()
                    content = "\n".join([c.content for c in chunks])
                
                if content:
                    doc_map[doc.file_hash] = content
