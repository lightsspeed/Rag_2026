...
export interface ChatMessage {
  type: "token" | "complete" | "error" | "status" | "sources" | "plan" | "doc_progress";
  content?: string;
  message?: string;
  filename?: string;
  step?: string;
...
export function createChatWebSocket(
  ...
  const token = getAccessToken();
  const wsUrl = `${WS_BASE}/ws/chat${token ? `?token=${encodeURIComponent(token)}` : ""}`;
...
export async function uploadDocument(file: File): Promise<{ filename: string; status: string; document_id: number }> {
...
export async function fetchDocumentPreviewUrl(filename: string): Promise<string> {
